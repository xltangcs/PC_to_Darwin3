...
void GPIO_init()
{
    int exportfd, directionfd_led, directionfd_tik, directionfd_reset;

    printf("GPIO init...\n");

    exportfd = open("/sys/class/gpio/export", O_WRONLY);

    write(exportfd, "961", 4); //led
    write(exportfd, "962", 4); //tik
    write(exportfd, "963", 4); //reset
    close(exportfd);

    directionfd_led   = open("/sys/class/gpio/gpio961/direction", O_RDWR);
    directionfd_tik   = open("/sys/class/gpio/gpio962/direction", O_RDWR);
    directionfd_reset = open("/sys/class/gpio/gpio963/direction", O_RDWR);

    write(directionfd_led  , "out", 1);
    write(directionfd_tik  , "out", 1);
    write(directionfd_reset, "out", 1);

    close(directionfd_led  );
    close(directionfd_tik  );
    close(directionfd_reset);
...
{
    int valuefd;
    valuefd = open("/sys/class/gpio/gpio963/value", O_RDWR);
    write(valuefd,"0", 2);
    sleep(1);
    write(valuefd,"1", 2);

    valuefd = open("/sys/class/gpio/gpio962/value", O_RDWR);
    write(valuefd,"0", 2);
